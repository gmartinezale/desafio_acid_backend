...
function getDataETH(data,client,API_KEY,callback){
	var key_redis = (data.period == 'diario') ? 'data_eth_diary':'data_eth_monthly';
	...
	client.get(key_redis,(err,val) => {
        if(val == null){ //NO EXISTE LLAVE
            var labelGraph = [];
            var dataGraph = [];
            URL = "https://www.alphavantage.co/query?function="+function_api+"&symbol=ETH&market=CLP&apikey="+API_KEY;
